...
use crate::rpc_types::{
    self, FetchMessageResult, JsonRpcMethod, JsonRpcParam, JsonRpcRequest,
};
...
pub struct Pairing<'a> {
    ...
    proposal_request: Option<rpc_types::session_propose::Params>,
    authenticate_request: Option<rpc_types::session_authenticate::Params>,
    // approve_done: bool,
}
...
impl<'a> Pairing<'a> {
    ...
    pub fn proposal_request(
        &self,
    ) -> Option<&rpc_types::session_propose::Params> {
        self.proposal_request.as_ref()
    }

    pub fn authenticate_request(
        &self,
    ) -> Option<&rpc_types::session_authenticate::Params> {
        self.authenticate_request.as_ref()
    }

    pub(crate) fn set_proposal_and_authenticate_request(
        &mut self,
        proposal_request: rpc_types::session_propose::Params,
        authenticate_request: rpc_types::session_authenticate::Params,
    ) {
    ...
    pub fn irn_fetch_messages_and_decrypt(&self) -> Result<Vec<JsonRpcParam>> {
        self.irn_fetch_messages()?
            ...
            .map(|m| -> Result<JsonRpcParam> {
                let request = JsonRpcRequest::decrypt(
                    &m.message,
                ...
                match request.method {
                    JsonRpcMethod::SessionPropose => Ok(
                        JsonRpcParam::SessionPropose(serde_json::from_value::<
                            rpc_types::session_propose::Params,
                        >(
                            request.params.ok_or("params not present")?,
                        )?),
                    ),
                    JsonRpcMethod::SessionAuthenticate => {
                        Ok(JsonRpcParam::SessionAuthenticate(
                            serde_json::from_value::<
                                rpc_types::session_authenticate::Params,
                            >(
                                request.params.ok_or("params not present")?
                            )?,
                        ))
...
mod tests {
    ...
    fn test_decode() {
        ...
        let result = JsonRpcRequest::decrypt(
            encoded,
